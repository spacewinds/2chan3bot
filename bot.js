  ...
  channel
    ...
    .then((messages) => {
      ...
      messagesArray.forEach((mItem) => {
        let currentTitle = mItem.embeds.length ? mItem.embeds[0].title : "";
        if (currentTitle) {
...
const sendNewPosts = (general) => {
  ...
  newPostsD.forEach((p) => {
    const embed = generatePost({
      post: {
        number: p.num,
        url: "https://2ch.hk/fag/res/" + currentThreadD.num + ".html#" + p.num,
        text: decodeHtmlCharCodes(striptags(decode(p.comment), [], "\n")),
            ...
            if (data) {
              sharp(backBuffer)
                ...
                .toBuffer((err2, data2, info2) => {
                  ...
                  channel.send(
                    "",
                    new Discord.Attachment(data2, "lukoshko.png")
                  );
                });
            }
          });
    ...
    if (gl) {
      ...
      gl.members.forEach((member) => {
        if (member.user.id === "666633875140902912") {
          member.send(guildname + channel + " " + tag + " " + content);
          if (attachments) {
  ...
  if (message && message.channel && message.channel.guild) {
    ...
    if (passportPlace) {
      switch (message.content.toLowerCase()) {
        ...
        case "хай":
        case "привет":
          const role = message.channel.guild.roles.find("name", "Citizen");
  ...
  if (content.substring(0, 1) == "-") {
    ...
    switch (cmd) {
      ...
      case "rp":
        ...
        break;
      case "scrape":
        scrapf(message.channel, args[0], args.length > 1 ? args[1] : undefined);
        break;
      case "stories":
      ...
      case "lolgame":
        checkIfUserIsInGame(text, (result) => {
          if (result)
            message.channel.send("Player " + text + " is in the game!");
          else message.channel.send("Player " + text + " is NOT in the game!");
        });
        break;
      case "_spyfox":
        const list_ = client.guilds.get("589192369048518723"); //589192369048518723
      ...
      case "_sfcrv":
        ...
        break;
      case "created":
        let user_ = message.guild.members.find((val) => val.user.tag === text);
        if (user_) {
          message.channel.send(
            user_.user.tag +
              " was created at " +
        ...
        break;
      case "avatar":
        user_ = message.guild.members.find((val) => val.user.tag === text);
        if (user_) {
      ...
      case "displayAvatar":
        user_ = message.guild.members.find((val) => val.user.tag === text);
        if (user_) {
          downloadAvatar(message.channel, user_.user.displayAvatarURL);
        }
        break;
      case "lukoshko":
        user_ = message.guild.members.find((val) => val.user.tag === text);
