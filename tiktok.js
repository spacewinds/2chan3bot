      ...
      .then((response) => {
        ...
        if (idx >= 0) {
          ...
          axios
            ...
            .then((rsp) => {
              extractIdFromUrl(rsp.request.res.responseUrl, onReady);
            });
        }
      })
      .catch((error) => {
...
export const loadMetaInfo = (id, onReady, url) => {
  axios
    .get("http://185.227.111.142/api/video/getAwemeWithUrl?url=" + url)
    .then(function (response) {
      const meta = response.data;
      if (meta && meta.video && meta.video.download_url) {
        onReady({
          desc: meta.video.description,
          video: meta.video,
        });
      }
    })
    .catch((error) => {
      console.log("OLD META INFO ERROR ", error);
    });
};
...
export const randomPost = (collection, username, onReady) => {
  if (collection && collection.collector) {
    const video = collection.collector[randomInt(collection.collector.length)];
    const link = "https://www.tiktok.com/@" + username + "/video/" + video.id;
  ...
  axios
    ...
    .then(function (response) {
      ...
      axios
        ...
        .then((iqrResult) => {
          const ibuf = require("buffer").Buffer.from(iqrResult.data, "binary");
          ...
          for (i = 0; i < result.collector.length; i++) {
            ...
            if (item.id === config.videoId) {
              axios
                ...
                .then((videoBuf) => {
                  const idx = videoBuf.data.indexOf("vid:", 0, "ascii");
